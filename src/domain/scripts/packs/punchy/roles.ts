...
export const punchyRoles: readonly SmartRoleDefinition[] = [
  ...
  role({ id: 'princess', name: '公主', officialName: 'Princess', team: 'townsfolk', abilityText: "On your 1st day, if you nominated & executed a player, the Demon doesn't kill tonight.", inputKinds: ['none'], possibleOutcomes: ['在公主（本人）的首个白天，如果公主提名并处决了一名玩家，当晚恶魔不会造成死亡；如果公主在游戏中途被创造，以她成为公主后的第一天计算。', '公主的能力不要求被处决的玩家死亡：公主提名的玩家不论由于何种原因被处决，都满足“提名并处决了一名玩家”的要求。', '公主的能力只阻止恶魔造成的死亡：恶魔依然会选择一名玩家，但他不会死亡；恶魔的其他能力（如中毒、错误信息）不受影响，非恶魔造成的夜间死亡照常发生。', '如果公主在白天醉酒但在夜晚恢复清醒，她能阻止恶魔造成的死亡；如果公主白天清醒但夜晚醉酒，她的能力不会生效。'], highRiskNotes: ['需要核对公主首个白天的提名人、处决是否成立（不要求被处决者死亡）、公主夜晚是否清醒和死亡来源是否为恶魔；不自动停刀。'] }),
