...
export const hideAndSeekRoles: readonly SmartRoleDefinition[] = [
  ...
  role({ id: 'preacher', name: '传教士', officialName: 'Preacher', team: 'townsfolk', abilityText: 'Each night, choose a player: a Minion, if chosen, learns this. All chosen Minions have no ability.', inputKinds: ['player'], stateChanges: ['被选中过的爪牙在传教士存活且清醒健康期间失去能力。', '如果传教士死亡或离场，被布道的爪牙恢复能力（已经使用了“每局游戏限一次”的能力的爪牙无法再次使用能力）。', '如果传教士醉酒或中毒，被布道的爪牙会重新获得能力，直到传教士恢复清醒健康后再次失去能力。'], playerMessageTemplates: ['你被传教士选中了，失去爪牙能力。'], highRiskNotes: ['只在目标实际为爪牙时通知目标；是否失去能力由说书人确认。', '如果传教士在醉酒或中毒的时候选择了一名玩家，那名玩家不会被传教士的能力影响。', '被标记的玩家角色不再是爪牙时，不再受传教士能力影响。'] }),
  ...
  role({ id: 'undertaker', name: '送葬者', officialName: 'Undertaker', team: 'townsfolk', abilityText: 'Each night*, you learn which character died by execution today.', inputKinds: ['role'], possibleOutcomes: ['告知今天被处决死亡玩家的角色。'] }),
  ...
  role({ id: 'seamstress', name: '女裁缝', officialName: 'Seamstress', team: 'townsfolk', abilityText: 'Once per game, at night, choose 2 players (not yourself): you learn if they are the same alignment.', inputKinds: ['players'], possibleOutcomes: ['告知两名目标是否同阵营。'] }),
  ...
  role({ id: 'huntsman', name: '巡山人', officialName: 'Huntsman', team: 'townsfolk', abilityText: 'Once per game, at night, choose a living player: the Damsel, if chosen, becomes a not-in-play Townsfolk. [+the Damsel]', inputKinds: ['player'], setupImpact: ['+落难少女，通常替换 1 名镇民。'], possibleOutcomes: ['若选中落难少女，目标变成一个不在场镇民。'], identityChanges: ['落难少女可能变成不在场镇民。'], highRiskNotes: ['救援成功后身份变化必须由说书人确认并追加更正记录。'] }),
  ...
  role({ id: 'virgin', name: '贞洁者', officialName: 'Virgin', team: 'townsfolk', abilityText: 'The 1st time you are nominated, if the nominator is a Townsfolk, they are executed immediately.', inputKinds: ['none'], possibleOutcomes: ['首次被镇民提名时，提名者立即被处决。'], stateChanges: ['可能立即处决提名者。'], highRiskNotes: ['确认提名者实际是否镇民，再决定是否触发。'] }),
